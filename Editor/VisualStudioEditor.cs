...
namespace Microsoft.Unity.VisualStudio.Editor
{
	...
	public class VisualStudioEditor : IExternalCodeEditor
	{
		...
		public void OnGUI()
		{
			...
			if (installation is VisualStudioCodeBuddyInstallation)
			{
				var reuseWindow = EditorPrefs.GetBool(VisualStudioCodeBuddyInstallation.ReuseExistingWindowKey, false);
				var newReuseWindow = EditorGUILayout.Toggle(new GUIContent("Reuse existing CodeBuddy window", "When enabled, opens files in an existing CodeBuddy window if found. When disabled, always opens a new window."), reuseWindow);
				if (newReuseWindow != reuseWindow)
					EditorPrefs.SetBool(VisualStudioCodeBuddyInstallation.ReuseExistingWindowKey, newReuseWindow);
